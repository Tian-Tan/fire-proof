...
export default function App() {
  ...
  const fireGeoJSON = dangerZones.length > 0 ? buildFireGeoJSON(dangerZones) : null;
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {routeGeoJSON && (
          <ShapeSource id="route" shape={routeGeoJSON}>
            <LineLayer
              id="routeCasing"
              style={{ lineColor: "#fff", lineWidth: 7, lineCap: "round", lineJoin: "round" }}
            />
            <LineLayer
              id="routeLine"
              style={{
        ...
        {destinationGeoJSON && (
          <ShapeSource id="destination" shape={destinationGeoJSON}>
            <CircleLayer id="destinationRing" style={{ circleRadius: 14, circleColor: "#007AFF", circleOpacity: 0.2 }} />
            <CircleLayer id="destinationDot" style={{ circleRadius: 8, circleColor: "#007AFF", circleStrokeWidth: 2, circleStrokeColor: "#fff" }} />
          </ShapeSource>
        )}
        ...
        {fireGeoJSON && (
          <ShapeSource id="fires" shape={fireGeoJSON}>
            <HeatmapLayer
              id="fireHeatmap"
              style={{
                heatmapRadius: ["interpolate", ["linear"], ["zoom"], 4, 1, 7, 4, 10, 12, 13, 25, 16, 40],
                heatmapIntensity: ["interpolate", ["linear"], ["zoom"], 4, 0.1, 9, 0.35, 13, 0.8],
                heatmapOpacity: 0.85,
                heatmapWeight: ["match", ["get", "risk_level"], "critical", 2.0, "high", 1.5, "medium", 1.0, 0.6],
                heatmapColor: [
                  "interpolate", ["linear"], ["heatmap-density"],
                  0, "rgba(0,0,0,0)",
                  0.1, "rgba(34,197,94,0.5)",
                  0.3, "rgba(132,204,22,0.65)",
                  0.5, "rgba(255,159,10,0.8)",
                  0.7, "rgba(255,69,58,0.9)",
                  0.9, "rgba(180,20,10,1)",
                  1.0, "rgba(100,0,0,1)",
                ],
              }}
            />
            <CircleLayer id="fireDot" style={{ circleRadius: 2, circleColor: "#fff", circleOpacity: 0.9 }} />
          </ShapeSource>
        )}
      ...
      {showNavigateButton && (
        <TouchableOpacity
          style={[styles.navigateButton, evacuationRecommended && styles.navigateButtonUrgent]}
